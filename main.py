...
class TextBekharBot:
    ...
    def _setup_handlers(self):
        ...
        self.bot.message_handler(commands=['makeadmin'])(self.handle_make_admin)
        self.bot.message_handler(commands=['myid'])(self.handle_my_id)
        self.bot.message_handler(commands=['send'])(self.handle_send_command)
        
        # Contact handler
        self.bot.message_handler(content_types=['contact'])(self.handle_contact)
        
        # Registration handlers
        self.bot.message_handler(func=self._is_registration_step('first_name'))(self.handle_first_name)
        self.bot.message_handler(func=self._is_registration_step('last_name'))(self.handle_last_name)
        self.bot.message_handler(func=self._is_registration_step('province'))(self.handle_province)
        
        # Main menu handlers - Professional layout
        self.bot.message_handler(func=lambda m: m.text == "🏠 صفحه اصلی")(self.handle_home)
        self.bot.message_handler(func=lambda m: m.text == "🔥 پر بازدید ترین ترک ها")(self.handle_top_tracks)
        self.bot.message_handler(func=lambda m: m.text == "💰 پکیج اقتصادی")(self.handle_economic_package)
        self.bot.message_handler(func=lambda m: m.text == "👑 پکیج مگاهیت VIP")(self.handle_vip_package)
        self.bot.message_handler(func=lambda m: m.text == "📞 ارتباط با ما")(self.handle_contact_us)
        self.bot.message_handler(func=lambda m: m.text == "ℹ️ درباره ما")(self.handle_about_us)
        self.bot.message_handler(func=lambda m: m.text == "👤 پنل کاربری")(self.handle_user_panel)
        
        # User panel handlers
        self.bot.message_handler(func=lambda m: m.text == "👤 اطلاعات من")(self.handle_user_info)
        self.bot.message_handler(func=lambda m: m.text == "📊 آمار من")(self.handle_user_stats)
        self.bot.message_handler(func=lambda m: m.text == "⚙️ تنظیمات")(self.handle_user_settings)
        self.bot.message_handler(func=lambda m: m.text == "📋 راهنما")(self.handle_guide)
        
        # Admin panel handlers - Professional layout
        self.bot.message_handler(func=lambda m: m.text == "👑 پنل ادمین")(self.handle_admin_panel)
        self.bot.message_handler(func=lambda m: m.text == "👤 پنل کاربر")(self.handle_user_panel)
        self.bot.message_handler(func=lambda m: m.text == "👋 خوش آمدید")(self.handle_welcome)
        self.bot.message_handler(func=lambda m: m.text == "📊 آمار کلی")(self.handle_general_stats)
        self.bot.message_handler(func=lambda m: m.text == "🔙 بازگشت")(self.handle_back_to_main)
        
        # User Management handlers
        self.bot.message_handler(func=lambda m: m.text == "👥 مدیریت کاربران")(self.handle_user_management)
        self.bot.message_handler(func=lambda m: m.text == "📋 لیست کاربران")(self.handle_list_users)
        self.bot.message_handler(func=lambda m: m.text == "➕ افزودن ادمین")(self.handle_add_admin_prompt)
        
        # Content Management handlers
        self.bot.message_handler(func=lambda m: m.text == "📁 مدیریت محتوا")(self.handle_content_management)
        self.bot.message_handler(func=lambda m: m.text == "🎵 افزودن موزیک")(self.handle_add_music_menu)
        self.bot.message_handler(func=lambda m: m.text == "📝 افزودن متن")(self.handle_add_text_menu)
        
        # System Settings handlers
        self.bot.message_handler(func=lambda m: m.text == "⚙️ تنظیمات سیستم")(self.handle_system_settings)
        self.bot.message_handler(func=lambda m: m.text == "🔧 ابزارها")(self.handle_system_tools)
        
        # Content addition handlers
        self._setup_content_handlers()
        
        # File handlers
        self.bot.message_handler(content_types=['audio', 'document'], func=self._is_admin_adding_music)(self.handle_admin_music)
        
        # Admin text input handlers
        self.bot.message_handler(func=self._is_admin_adding_text)(self.handle_admin_text)
        
        # Admin ID input handler
        self.bot.message_handler(func=self._is_admin_inputting_id)(self.handle_admin_id_input)
        
        # Callback query handlers for user management
        self.bot.callback_query_handler(func=lambda call: call.data.startswith('user_list_'))(self.handle_user_list_callback)
        self.bot.callback_query_handler(func=lambda call: call.data.startswith('user_detail_'))(self.handle_user_detail_callback)
        self.bot.callback_query_handler(func=lambda call: call.data.startswith('ban_user_'))(self.handle_ban_user_callback)
        self.bot.callback_query_handler(func=lambda call: call.data.startswith('unban_user_'))(self.handle_unban_user_callback)
        self.bot.callback_query_handler(func=lambda call: call.data.startswith('make_admin_'))(self.handle_make_admin_callback)
        self.bot.callback_query_handler(func=lambda call: call.data.startswith('make_user_'))(self.handle_make_user_callback)
        self.bot.callback_query_handler(func=lambda call: call.data.startswith('user_stats_'))(self.handle_user_stats_callback)
        self.bot.callback_query_handler(func=lambda call: call.data.startswith('message_user_'))(self.handle_message_user_callback)
        self.bot.callback_query_handler(func=lambda call: call.data == 'user_search')(self.handle_user_search_callback)
        self.bot.callback_query_handler(func=lambda call: call.data.startswith('search_by_'))(self.handle_search_type_callback)
        
        # Search input handlers
        self.bot.message_handler(func=self._is_admin_searching)(self.handle_search_input)
        
        # Admin message input handler
        self.bot.message_handler(func=self._is_admin_sending_message)(self.handle_admin_message_input)
    ...
    def _setup_content_handlers(self):
        ...
        content_actions = [
            # Top tracks content
            ("افزودن موزیک به پر بازدید ترین ترک ها 🎵🔥", ContentCategory.TOP_TRACKS, "music"),
            ("افزودن متن به پر بازدید ترین ترک ها 📝🔥", ContentCategory.TOP_TRACKS, "text"),
            # Economic package content
            ("افزودن موزیک به پکیج اقتصادی 🎵💰", ContentCategory.ECONOMIC_PACKAGE, "music"),
            ("افزودن متن به پکیج اقتصادی 📝💰", ContentCategory.ECONOMIC_PACKAGE, "text"),
            # VIP package content
            ("افزودن موزیک به پکیج مگاهیت VIP 🎵👑", ContentCategory.VIP_PACKAGE, "music"),
            ("افزودن متن به پکیج مگاهیت VIP 📝👑", ContentCategory.VIP_PACKAGE, "text"),
        ]
        
        for button_text, category, content_type in content_actions:
            handler_func = lambda m, c=category, t=content_type: self.handle_add_content(m, c, t)
            self.bot.message_handler(func=lambda m, text=button_text: m.text == text)(handler_func)
    
    def _is_registration_step(self, step: str):
        """Check if user is in specific registration step"""
        def check(message):
            session = self.session_manager.db.get_session(message.from_user.id) or {}
    ...
    def handle_start(self, message):
        ...
        try:
            ...
            self.bot.send_message(message.chat.id, self.formatter.format_error_message())
    ...
    def handle_make_admin(self, message):
        ...
        try:
            ...
            if success:
                self.bot.send_message(message.chat.id, "شما به عنوان ادمین اصلی اضافه شدید. ✅")
            else:
                self.bot.send_message(message.chat.id, self.formatter.format_error_message())
    ...
    def handle_send_command(self, message):
        ...
        try:
            if not self.db.is_admin(message.from_user.id):
                self.bot.send_message(message.chat.id, self.formatter.format_error_message("permission_denied"))
                return
            ...
            try:
                ...
                self.bot.send_message(message.chat.id, "❌ شناسه کاربری باید عدد باشد.")
                return
            ...
            self.session_manager.start_admin_action(message.from_user.id, 'send_message')
    ...
    def handle_contact(self, message):
        ...
        try:
            ...
            if not self.validator.validate_phone_number(phone):
                self.bot.send_message(message.chat.id, self.formatter.format_error_message("invalid_input"))
                return
            
            # Update registration data
            self.session_manager.update_registration_data(user_id, {'phone': phone})
            self.session_manager.update_registration_step(user_id, 'first_name')
    ...
    def handle_first_name(self, message):
        ...
        try:
            ...
            self.session_manager.update_registration_data(user_id, {'first_name': first_name})
            self.session_manager.update_registration_step(user_id, 'last_name')
            
            self.bot.send_message(message.chat.id, Messages.FIRST_NAME_RECEIVED)
    ...
    def handle_last_name(self, message):
        ...
        try:
            ...
            self.session_manager.update_registration_data(user_id, {'last_name': last_name})
    ...
    def handle_province(self, message):
        ...
        try:
            ...
            if success:
                ...
                self.bot.send_message(message.chat.id, self.formatter.format_error_message("database_error"))
...
📞 برای اطلاعات بیشتر با ما تماس بگیرید."""
    ...
    def handle_general_stats(self, message):
        ...
        try:
            ...
            admins = len([u for u in users if u['role'] in ['admin', 'super_admin']])
...
🔙 برای بازگشت از دکمه بازگشت استفاده کنید."""
    ...
    def _handle_content_request(self, message, category: str):
        ...
        try:
            ...
            # Send text content
            text_content = self.formatter.format_content_list(contents, category_display)
            ...
            for music_content in contents.get('music', []):
                if music_content.get('file_id'):
                    try:
                        self.bot.send_document(message.chat.id, music_content['file_id'])
...
• در صورت مشکل با پشتیبانی تماس بگیرید"""
    ...
    def handle_list_users(self, message):
        ...
        try:
            ...
            message_text = self.formatter.format_professional_user_list(users, 1, total_pages, total_users)
            keyboard = self.keyboard_manager.get_user_list_keyboard(users, 1, total_pages)
            
            self.bot.send_message(message.chat.id, message_text, reply_markup=keyboard, parse_mode='Markdown')
    ...
    def handle_add_admin_prompt(self, message):
        ...
        self.session_manager.start_admin_action(message.from_user.id, 'add_admin')
        self.bot.send_message(
            message.chat.id, 
            "🛡️ افزودن ادمین جدید\n\n"
    ...
    def handle_add_content(self, message, category: str, content_type: str):
        ...
        self.session_manager.start_admin_action(message.from_user.id, f'add_{content_type}', category)
        
        if content_type == 'music':
            self.bot.send_message(message.chat.id, "لطفا فایل موزیک را ارسال کنید. 🎵")
        else:
            self.bot.send_message(message.chat.id, "لطفا متن اولیه را وارد کنید. 📝")
    ...
    def handle_admin_music(self, message):
        ...
        try:
            ...
            if message.audio:
                ...
                self.bot.send_message(message.chat.id, "لطفا فایل موزیک ارسال کنید.")
                return
            ...
            self.bot.send_message(message.chat.id, "موزیک دریافت شد. حالا لطفا متن اولیه را وارد کنید. 📝")
    ...
    def handle_admin_text(self, message):
        ...
        try:
            ...
            if success:
                self.bot.send_message(message.chat.id, self.formatter.format_success_message("content_added"))
            else:
                self.bot.send_message(message.chat.id, self.formatter.format_error_message("database_error"))
    ...
    def handle_default(self, message):
        ...
        self.bot.reply_to(message, "لطفا از دستور /start استفاده کنید تا ثبت نام کنید. 🤖")
    ...
    def handle_user_list_callback(self, call):
        ...
        try:
            if not self.db.is_admin(call.from_user.id):
                self.bot.answer_callback_query(call.id, "شما دسترسی لازم را ندارید. ❌")
                return
            
            data_parts = call.data.split('_')
            if len(data_parts) >= 3:
                page = int(data_parts[2])
                search = data_parts[3] if len(data_parts) > 3 and data_parts[3] else None
            else:
                page = 1
                search = None
            
            result = self.db.get_users_paginated(page=page, per_page=10, search=search)
            users = result['users']
            total_users = result['total']
            total_pages = result['total_pages']
            
            message_text = self.formatter.format_professional_user_list(users, page, total_pages, total_users, search)
            keyboard = self.keyboard_manager.get_user_list_keyboard(users, page, total_pages, search)
    ...
    def handle_ban_user_callback(self, call):
        ...
        try:
            ...
            if success:
                ...
                self.bot.answer_callback_query(call.id, "خطا در بن کردن کاربر. ❌")
    ...
    def handle_unban_user_callback(self, call):
        ...
        try:
            ...
            if success:
                ...
                self.bot.answer_callback_query(call.id, "خطا در آزاد کردن کاربر. ❌")
    ...
    def handle_make_admin_callback(self, call):
        ...
        try:
            ...
            if success:
                self.bot.answer_callback_query(call.id, "کاربر به ادمین تبدیل شد. 🛡️")
                # Refresh the user detail view
                self.handle_user_detail_callback(call)
            else:
                self.bot.answer_callback_query(call.id, "خطا در تبدیل کاربر به ادمین. ❌")
    ...
    def handle_make_user_callback(self, call):
        ...
        try:
            ...
            if success:
                self.bot.answer_callback_query(call.id, "کاربر به کاربر عادی تبدیل شد. 👤")
                # Refresh the user detail view
                self.handle_user_detail_callback(call)
            else:
                self.bot.answer_callback_query(call.id, "خطا در تبدیل کاربر به کاربر عادی. ❌")
    ...
    def handle_message_user_callback(self, call):
        ...
        try:
            ...
            # Start message session
            self.session_manager.start_admin_action(call.from_user.id, 'send_message')
            ...
            self.bot.answer_callback_query(call.id, "حالا پیام خود را ارسال کنید")
    ...
    def handle_search_type_callback(self, call):
        ...
        try:
            ...
            search_type = call.data.split('_')[2]  # name, phone, province, role
            
            # Start search session
            self.session_manager.start_admin_action(call.from_user.id, 'search_users')
            self.session_manager.update_admin_session(call.from_user.id, {
            ...
            prompt = search_prompts.get(search_type, 'لطفا عبارت جستجو را وارد کنید:')
    ...
    def handle_search_input(self, message):
        ...
        try:
            ...
            if search_type == 'role':
                ...
                result = self.db.get_users_paginated(page=1, per_page=50, search=None)
                # Filter by role
                filtered_users = [user for user in result['users'] if user['role'] == search_term]
                result['users'] = filtered_users
                result['total'] = len(filtered_users)
            else:
                result = self.db.get_users_paginated(page=1, per_page=50, search=search_term)
            ...
            if not users:
                ...
                message_text = self.formatter.format_professional_user_list(users, 1, total_pages, total_users, search_term)
                keyboard = self.keyboard_manager.get_user_list_keyboard(users, 1, total_pages, search_term)
    ...
    def handle_admin_message_input(self, message):
        ...
        try:
            ...
            if not message_text:
                self.bot.send_message(message.chat.id, "❌ پیام نمی‌تواند خالی باشد.")
                return
